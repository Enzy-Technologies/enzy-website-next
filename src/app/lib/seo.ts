...
// Build the absolute OG image URL for a page. All cards are generated by the
// /og route so they share one cream + pixel-canvas look:
//   - An explicit imagePath always wins (lets a page pin a custom image).
//   - Home gets the centered brand card (variant=home).
//   - Every other page gets a left-aligned card rendered from its own
//     title + description, so each unique page gets its own preview.
